      ...
      <el-col :span="6">
        ...
        <el-row class="property-list">
          <el-card
            ...
            :style="{
              'background-color':
                item.pid === isSelected ? 'rgba(154,174,195,0.45)' : '',
            }"
          >
            <el-row>
              <div>
              ...
              <el-row type="flex" justify="end">
                ...
                >
                <el-button
                  v-show="item.status === 'R'"
                  type=""
                  plain
                  round
                  icon="el-icon-close"
                  @click="cancelAuc(item)"
                  >Cancel</el-button
                >
              </el-row>
            </el-row>
        ...
        >
          ...
          <el-dialog title="Register New Auction:" :visible.sync="Aucreg">
                ...
                <el-col :span="24">
                  ...
                  <el-form-item label="Minimum Price:" prop="minimumPrice">
                    <el-input v-model="form.minimumPrice">
                      <i slot="suffix" class="input-slot"
                        >{{ form.minimumPrice | numFormat }} A$</i
                      >
                    </el-input>
                  </el-form-item>
                </el-col>
            ...
            <div slot="footer" class="dialog-footer">
              ...
              <el-button type="primary" @click="submitReg(propInfo.pid)"
                >Submit</el-button
              >
            </div>
          </el-dialog>
          ...
          <section
            class="mh20"
            v-if="propInfo.status === 'A' || propInfo.status === 'R'"
          >
            <h5>Auction</h5>
          ...
          <section class="mh20" v-else>
            ...
            <el-row type="flex" justify="front">
              <el-button
                ...
                @click="aucreg"
                >Register New Auction</el-button
              >
            </el-row>
          </section>
        </el-row>
...
export default {
  ...
  created() {
    ...
    this.propInfo = this.originPropertyList[0];
    this.$title = "Management: " + this.propInfo.address;
  },
  ...
  methods: {
    ...
    removeItem(item) {
      this.$confirm("Remove this property?", "Alert", {
        ...
        data.append("pid", item.pid);
        data.append("aid", item.aid);
        this.$axios.post("/property/delete" + data).then((response) => {
          if (response.status >= 200 && response.status < 300) {
            if (response.data.code === 200) {
              this.$message({
                type: "success",
