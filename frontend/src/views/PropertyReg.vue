      ...
      >
          ...
          <el-row type="flex" justify="center">
            <el-col :span="15" style="align-items: center">
              <label class="submit-label"
                >Please upload your property photos here, no more than 5
                photos.</label
              >
            </el-col>
          </el-row>
                  ...
                  <el-col :span="24">
                    ...
                    >
                      <el-input v-model="form5.minimumPrice">
                        <i slot="suffix" class="input-slot"
                          >{{ form5.minimumPrice | numFormat }} A$</i
                        >
                      </el-input>
                    </el-form-item>
                  </el-col>
        ...
        <el-tab-pane label="Submit" :disabled="dis5" name="5">
          <el-form :model="form" label-width="80px" label-position="left">
            <el-row type="flex" justify="center">
              <label style="font-size: 20px; margin: 80px 0; font-weight: bold"
                >Congratulations! You have finished a property
                registration.</label
              >
            </el-row>
            <el-row type="flex" justify="center">
...
export default {
  ...
  data() {
    ...
    const checkStart = (rule, value, callback) => {
      ...
      if (!intReg.test(value)) {
        ...
        callback(
          new Error("Minimum price should smaller than reserved price!")
        );
      } else {
        callback();
      }
    ...
    return {
      ...
      rules: {
        bedroomNum: [
          {
            required: true,
            message: " Please enter bedroom number",
            trigger: "blur",
          },
        ...
        garageNum: [
          {
            required: true,
            message: " Please enter garage number",
            trigger: "blur",
          },
          { validator: checkInt, trigger: "blur" },
        ],
  ...
  methods: {
    ...
    submit() {
      ...
      this.$axios
        .post("/property/registration", data)
        .then((response) => {
          if (response.status >= 200 && response.status < 300) {
            if (response.data.code === 200) {
  ...
  watch: {
    place: function (newPlace, oldPlace) {
      ...
      if (country !== "Australia") {
        this.$message.error(
          "Propery for registration must locate in Australia!!"
        );
        setTimeout(function () {
          // location.reload();
        }, 2000);
